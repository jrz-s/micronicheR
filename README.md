...
Mechanistic physiological niche modeling based on microclimate
simulations
...
- reuse the same microclimate simulation across species, substantially
  reducing computational time in multispecies analyses;
...
Run the following command once before using `endonicheR()` or
`ectonicheR()`:
...
The package includes example rasters, coordinates, and species trait
datasets that can be used to reproduce the examples below.
...
The `endonicheR()` and `ectonicheR()` functions support the same three
output formats. For simplicity, the examples below use `endonicheR()`
only.
...
Both `endonicheR()` and `ectonicheR()` support large simulations through
the break_n argument. The examples below use `endonicheR()`, but the
same workflow applies to `ectonicheR()` because both functions share the
same checkpoint system and arguments.
...
For `endonicheR()`, the checkpoint directory has the following
structure:
...
For `ectonicheR()`, the checkpoint directory has the following
structure:
...
For each processed block, `endonicheR()` stores independent checkpoints
for:

- microclimate simulations generated by `micro_global()`;
- physiological simulations generated by `endoR()`.

These checkpoints allow previously completed stages to be reused without
recomputation, substantially reducing execution time when simulations
are resumed after an interruption.
...
This permanently removes all checkpoint files generated during previous
simulations.

## Disclaimer

micronicheR is under active development. Although the package has been
extensively tested, interfaces and functionality may change in future
releases. Users are encouraged to report bugs and suggestions through
