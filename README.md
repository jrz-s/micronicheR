...
modeling based on microclimate simulations generated by the `NicheMapR`
package. The current release provides the `endonicheR()` workflow for
endotherms, whereas the complementary `ectonicheR()` workflow for
ectotherms is currently under active development. The package is
...
`See` “Run `endonicheR` using a study raster” and “Run `endonicheR`
using coordinates.”
...
`micronicheR` is under active development. The current release provides
endotherm physiological niche modeling through `endonicheR()`, while
additional mechanistic workflows, including `ectonicheR()` for
ectotherms, are under active development.
...
Before running `micronicheR::endonicheR()` for the first time, users
...
Large spatial simulations involving thousands of pixels or multiple
species can be computationally demanding. To improve robustness,
`endonicheR()` allows simulations to be divided into processing blocks
through the `break_n` argument.
