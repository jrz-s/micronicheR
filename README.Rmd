...
This permanently removes all saved checkpoint files generated during previous simulations.

## Disclaimer

`micronicheR` is currently under active development and should be considered experimental software. Functions, arguments, outputs, and workflows may change in future releases as new features are implemented and existing functionalities are improved.
...
Before running `micronicheR::microniche()` for the first time, users should download and register the required global climate data:
...
For large simulations involving thousands of pixels, coordinates, or multiple species, users are strongly encouraged to use the break_n argument to divide computations into smaller processing blocks. This strategy can improve computational stability, reduce memory consumption, and minimize the risk of losing progress due to unexpected interruptions during long simulations.
